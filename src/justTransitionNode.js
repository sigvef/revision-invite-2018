(function(global) {
  class justTransitionNode extends NIN.ShaderNode {
    ...
    update(frame) {
      ...
      this.textCtx.fillRect(0, 0, 16 * GU, easeOut(-2 * GU, 9 * GU, t));
      this.textCtx.fillStyle = '#0000ff';
      this.textCtx.fillRect(0, easeOut(0, 11 * GU, t), 16 * GU, 9 * GU);
      this.textCtx.fillStyle = '#77e15d';
      this.textCtx.fillRect(0, easeOut(-2 * GU, 9 * GU, t), 16 * GU, 2 * GU);
      this.textCtx.fillStyle = 'white';
      this.textCtx.fillText('JUST', 8 * GU, easeOut(-0.6 * GU, 10.165 * GU, t));
